...
> **視覺系統 A ✅ B ✅ C ✅ D ✅ E ✅** —— 擴充功能、分享報告頁、11 個官網頁全部換完，`homePage` 到 `reportsPage` 整段掃描 **emoji 0 個**。
> Worker bundle gzip **770 KiB**（PM-434 內嵌實拍蜂 base64，+約 71 KiB）。
> ⚠ `_verify_phase1` 照舊 **57 / 0**（瀏覽器段 SKIP，port 19850 被本 session 的 MCP server 占用）。

**卡片上的兩個數字我改了框架，值本身沒錯。** 卡片寫「端到端 961 → 999（+38），斷言 242 → 280（+38）」，961／242 是 **PM-433 做完之後**的值，不是 Day 51 的起點。日對日應該是 **938 → 999（+61）**、**219 → 280（+61）**。順帶一提，Day 50 區塊記的 937 少算一條 —— 14 套裡「非 `_verify403`」的部分今天量了五次都是固定的 719，719 + 219 = 938。

### 六張卡

| 卡 | 內容 |
|---|---|
| PM-432 | 分享報告頁（畫面 14/15）：黃底 + 黑 header + 分頁列 + Console/Info 兩分頁；分享連結卡移到標題右側 |
| PM-433 | 付費牆（畫面 16）+ 找不到報告（畫面 17）：66×76 幾何鎖頭、三格蜂巢滿／空／滿 |
| PM-434 | 官網共用外殼（nav/footer/色票/字型）套進 11 頁 + 首頁全頁改版（畫面 28） |
| PM-435 | 內容頁 6 頁（畫面 29）：FAQ 手風琴／更新日誌版本條目／隱私長文／功能／指南／AI 客服手冊 |
| PM-436 | 部落格／心得／問題回報／日票成功頁（畫面 30） |
| PM-437 | 收工：CHANGELOG + ARCHITECTURE |

### 這兩張群組 E 的卡，真正吃掉時間的是「翻譯字典」

官網有三本 `Record<string, string>` 字典（JA/KO/VI），`makeT(lang)` 用**繁體原文當 key** 去查。這造成兩層坑，兩層都踩過：

1. **改頁面必須同步改 key。** 清掉標題的 emoji 之後，`t('📌 關於產品', …)` 變成 `t('關於產品', …)`，字典裡的 key 還是舊的 → 日／韓／越三語整句掉回英文。同步了 21 條。
2. **🔴 字典的「值」也要清。** `makeT('ja')` 回傳的是**值**，不是 key —— 只清 zh/en 的話，日文訪客看到的照樣是 `🔧 無料インストール`。這一層是被自己的護欄抓出來的：那條斷言原本用 `slice(indexOf('const JA_MAP'), indexOf('function makeT'))` 取字典，但 `makeT` 定義在字典**前面**，切出空字串，斷言永遠綠。修好切片後立刻紅，挖出 **102 條**帶 emoji 的值。

### 順手修掉的三個既有問題

- **付費牆的「了解會員方案」是死連結** —— 指向 `/#pricing`，但站上從來沒有這個錨點。PM-434 補上定價區並掛 `id="pricing"`。
- **hreflang 宣告與語言切換不一致** —— privacy／skill／guide／changelog／feedback 宣告 `LANGS_3`（含简），頁面卻只有 zh/en 兩向切換。現在 nav 直接吃該頁的 `LANGS_*`，並加了跨函式斷言：11 頁的 `siteNav(lang, LANGS_x)` 必須等於自己的 `hreflangTags(…, LANGS_x)`。
- **`.error-msg` 那種只換文字顏色的狀態表達** —— 依 §7.7 改成「形狀／底色也不同」：找不到報告換成整頁狀態畫面，回報表單成功是黃底黑字、失敗是磚紅底。

### 三個「設計稿要、但資料沒有」的欄位，沒有編出來

| 元件 | 缺什麼 |
|---|---|
| 部落格分類膠囊 | `BlogPost` 沒有分類欄位，要做得先定分類清單並回填 5 篇文章 |
| 心得的職稱／使用時間 | `Testimonial` 只有 `name/text/videoUrl`（六角頭像取名字首字是做得到的，就做了） |
| 日票倒數「23:59:41 剩餘」 | `GET /day-pass-success` 是綠界的 ClientBackURL，沒有使用者身分、查不到 `day_pass_expires_at`。用 `now+24h` 前端倒數的話，隔一小時重整就從 24:00:00 重來 —— 那是騙人的，改成「24 小時無限使用」 |

三處都**沒有留下沒人用的 CSS**。

### 自己弄出來、也自己修掉的兩個

- **清 emoji 的腳本把原始碼縮排壓掉。** `clean()` 裡有一行全域的 `'  '→' '`，而含 `→` 的行也會進 `clean()`（`→` 在 emoji 偵測範圍內，只是被白名單保住不刪），那些行的縮排被砍半。PM-436 改掉腳本，並把 PM-435 那版誤壓的 19 行一起還原（純空白，HTML 會自己收合、畫面沒差）。
- **反向測試腳本把 `server/src/index.ts` 截斷成 0 bytes。** Python 的 `open(P,'w').write(x)` 是**先開檔（截斷）才求值 `x`**，那次 `x` 丟了例外，未 commit 的改動就沒了。用四支 patch 腳本從 HEAD 完整重建。之後所有腳本一律「先算完字串、算完才開檔」，並加長度下限保護。

### 兩條照 DESIGN_SPEC、不照設計稿的（§2.4 深底次要文字只有兩階）

設計稿在黑底上用了 `#6B5A3D`（2.1:1，footer 版權）和 `#8A7550`（3.8:1，月費卡的「/月」），兩個都在 §2.4 的禁用名單裡 → 一律改 `--on-dark #A08B62`。反過來，Network 表的 4xx 用 `#8A5A24` 是**對的** —— §2.4 的禁用只針對深底，原文還明講那些是「給黃底／米白底的褐色」，那張表在 `#FFF4D6` 上對比約 6.9:1。

### 護欄新增 61 條，反向測試 81/81 全紅

每張卡都把改動一條條破壞掉、確認斷言真的會紅。過程中抓到自己四條假斷言：

| 假在哪 | 修法 |
|---|---|
| `.tab-badge.error` 被 `.tab-btn.active .tab-badge.error` 餵飽，改壞 active 態照樣綠 | 錨定行首 + 另外要求 active 態那條 |
| 「黃底」只查 `background:var(--y)` 有沒有出現，但六角、複製鈕都是黃的 | 錨定 `body { background:var(--y); background-image:url("data:image/svg+xml,` |
| 「日期在標題上方」用 `indexOf(A) < indexOf(B)`，節點刪掉時 `indexOf` 回 **-1**，比誰都小 | 先確認兩邊都存在 |
| 字典斷言切片切到 `function makeT`（定義在字典前面）→ 永遠是空字串 | 改切到 `VI_MAP` 結尾；修好後立刻抓出 102 條 emoji 值 |

另外把 emoji 偵測範圍補上國旗（區域指示符號 `U+1F1E6–U+1F1FF`）—— 原本從 `U+1F300` 起算，`🇹🇼🇬🇧` 這類整段漏掉，補上後全 14 套重跑仍 0 failed。

## Day 50（2026-09-03）— PM-423~431，9 張卡：大黃蜂視覺系統 群組 A 修正 + 群組 B + 群組 C

> 全套 **937 / 0**（14 套）。`_verify403` **122 → 219**（+97 條規格護欄）。
> **MCP 工具總數 65 支不變**（bridge 51 + 雲端 14）——這兩天只換視覺，沒有動任何 API、事件綁定或狀態機。
> ⚠ `_verify_phase1` 照舊是 **57 / 0**（瀏覽器段 SKIP，port 19850 被本 session 的 MCP server 占用）。

**擴充功能端到這裡全部改完**（群組 A popup + B 擴充頁面 + C 頁內注入 UI）。剩下 server 端的群組 D（分享報告頁）與 E（官網 11 頁）。

### 九張卡

| 卡 | 內容 |
|---|---|
| PM-423 | 清除 popup 殘留的 9 個舊紫色系 token |
| PM-424 | 登入頁移除靜態 fallback + 加強影片對比度 |
| PM-425 | 付款中繼頁 `checkout` + `day-pass-checkout`（畫面 07） |
| PM-426 | 麥克風授權頁 `mic-permission`（畫面 08） |
| PM-427 | 截圖標注頁 `annotate`（畫面 05） |
| PM-428 | 報告編輯頁 `edit-report`（畫面 06） |
| PM-429 | `content.ts` 注入 UI（畫面 23/24） |
| PM-430 | `inject.ts` 注入 UI（畫面 23/25/26/27） |
| PM-431 | 收工：CHANGELOG + ARCHITECTURE |

### 這兩天真正的工作量不是換色碼，是「圖示被 textContent 洗掉」

把 emoji 換成幾何圖示之後，**每一處用 `textContent` 寫字的地方都會把圖示一起洗掉**。這在六個檔案裡出現了十幾次，是這兩天最大的一塊：

| 檔案 | 被洗掉的地方 |
|---|---|
| `annotate.ts` | 5 顆工具鈕（`data-i18n` 掛在 `<button>` 上）、錄音鈕 `'⏹'/'🎤'`、鍵盤切換鈕、下一步鈕、敏感警示條 |
| `edit-report.ts` | 播放鈕（4 個呼叫點）、語音鈕、標記刪除鈕、複製連結鈕、乾淨/原始切換 |
| `content.ts` | 釘選橫幅、描述卡標題、麥克風降級提示條 |
| `inject.ts` | 監控徽章、重啟鈕、收合鈕 |
| `mic-permission.ts` | 狀態膠囊的標記、底部提示的六角 |

三種解法，依情況選：**① 圖示做成兄弟節點**（最常用）**② 改切 class 由 CSS 決定顯示哪個圖示**（狀態切換用）**③ 用 `::before`**（`.ts` 會 `appendChild` 到同一個容器時只能這樣）。

其中 `.sensitive-tip::before` 還有第二層陷阱：`.ts` 寫的是 `tip.style.display = 'block'`，**inline style 蓋掉 flex**，而 block 容器裡的 `::before` 預設是 inline、寬高會被忽略 —— 必須寫 `display: inline-block`。

### 🔴 跨界協定：emoji 留在資料裡，只在畫面上剝掉

`content.ts` 的 `PIN_STATUS_EMOJI`（🟢🟡🔴⚪）與 `probeSummary()` 開頭的 🔴／⚠／✅ **不能刪**——它們是 content script → popup 的判定協定，popup 的 `sevClass()`（PM-418）靠開頭那顆 emoji 決定色條顏色。PM-405 定的規矩是「popup 不重新解讀 JSON 去猜嚴重度，兩邊各算一次遲早會分岔」。

作法：新增 `stripSev()`，**資料照舊、只在 content.ts 自己要把字串畫進頁面時剝掉**；狀態改用 §7.7 的左側 3px 色條表示。

### §2.2 比例反轉：注入 UI 是黑殼 + 黃強調

底下是別人的網站，整條黃橫幅會蓋掉對方設計。所以 `content.ts` / `inject.ts` 的所有元件一律**黑殼 + 黃強調**，**唯一例外是截圖工具列**——全寬且短暫存在的模態工具，用黃底宣告「現在是 BugEzy 在控制」。

### 字幕條的注意力階梯

§3.4 只規定了字級，但三條字幕條的**邊框強調程度**也應該分級（已寫進註解並釘成斷言，否則很容易被統一成同一種）：

| 條 | 字級 | 邊框 | 意思 |
|---|---|---|---|
| Whisper 錄音 | 16px | 1.5px **黃** | 正在錄，講完要按停止 |
| 即時字幕 | **17px** | 1.5px 咖啡 | 邊操作邊講，餘光掃過 |
| 鍵盤模式 | 16px | 1.5px `--line-dark-2` | 只是告知語音關著 |

### 依 DESIGN_SPEC 修正卡片／設計稿的地方

規範與卡片衝突時一律以 `DESIGN_SPEC.md` 為準（交付包 README 自訂的規則）：

- **`#8A7550` 被 §2.4 點名禁止**（3.8:1，「這個錯誤犯過三次」），卡片與設計稿有三處寫它 → 全改 `#A08B62`。
- **`#4A2F12` 不能當黃底說明文字**（§3.2 明講「不是 #4A2F12」）→ 改 `#3A2409`；字重一律補到 ≥600。
- **`乾淨模式` checkbox 的 `accent-color` 不能是 `#14110B`** —— 卡片這樣寫，但同一段也寫控制列背景是 `#14110B`，黑底上的黑勾等於看不見 → 改黃色。
- **`#stopBtn` 沒有改成磚紅**（PM-423 卡片要求）：設計稿畫面 03 明確畫的是黃底停止鈕、§2.1 寫明 `--err` 是給 error 用的、而磚紅壓在近黑底上幾乎沉下去。**待 FOX 決定。**

### 兩個「元件不存在」的發現

- **錄製工具列（畫面 23）不存在。** PM-429 卡片要求改「頂部固定的錄製工具列：計時器 + 停止 + 重新開始」，但 `content.ts` 與 `inject.ts` 都沒有這個元件（`grep elapsed|計時|00:00` 零筆）。錄製中的狀態指示目前是**擴充功能圖示上的 `REC` badge** + popup。要做是新元件不是換視覺。
- **AUTO 時間軸標記不存在。** PM-428 卡片要求 AUTO 標記用虛線，但 `TimeMarker` 只有 `{ time_sec, note }`，沒有欄位能區分手動與自動。**沒有先把 CSS 寫著放**——沒東西會加那個 class，那就是死 CSS。

### 兩個卡片描述與實際不符

- `inject.ts` **沒有用 Shadow DOM**（卡片背景這樣寫）。所有元件是普通 DOM 掛在 `document.body`，會**繼承所在網站的 CSS** —— 所以字體字重都寫死在 `cssText` 裡，不靠繼承。
- `annotate` 的 `<input type="color">` / `<select>` **不能移除**（卡片要求換成 CSS 圓點與橫條）：`annotate.ts` 的 `$()` 找不到元素會 `throw`，整頁會死。改成兩者留在 DOM 當唯一真相，色票只是把值寫回去的快捷鍵；另外**保留原生自訂色**避免功能倒退。

### 順手清掉的既有問題

- **`popup.ts` 六行舊按鈕宣告是死碼**：`#stopBtn`／`#copyBtn`／`#exportBtn`／`#clearBtn` 在 PM-416 就被下方規則整組蓋掉（同 `#id` 特異度、後定義者勝），畫面從未渲染過。整段刪掉而不是換色 —— 留著改色只會變成「同一顆按鈕兩條規則、兩個顏色」。
- **CSS 特異度陷阱**：`annotate` 的色票與粗細橫條也是 `<button>`，`.swatch`（0,1,0）壓不過 `.toolbar button`（0,1,1），會被套上工具鈕的內距與黑邊框。改用 `.toolbar button.swatch`（0,2,1）並加斷言。
- **inline style 會永久蓋掉 CSS**：`annotate` 與 `inject` 的音量條原本每一幀 inline 寫綠/紅，改成建立時依位置給色。
- **約 25 句硬寫在 `.ts` 的中文**（上傳狀態、轉錄狀態、鍵盤模式提示、更新通知…）在英文／日韓越模式下都只顯示中文，全部收進字典。
- **`edit-report` 摘要格**從「標籤：值」改成「值在上、標籤在下」，Network 那格標磚紅（唯一「有數字就是壞消息」的一格）。

### i18n

清了 **100 個以上** popup／annotate／edit-report／content／inject 可觸及的 key 的 emoji。其中**六個 key 的 emoji 是當按鈕代稱在用**（「按 🎤 錄音描述，講完按 ⏹ 轉錄」「可用 🔒 馬賽克筆刷塗掉」），直接刪會變破句 —— 這幾個是五語**整句改寫**，不是剝字元。

### 97 條新護欄（122 → 219）

每一張卡的護欄都做過**反向測試**（把改動破壞掉，確認斷言真的會紅）。PM-425 那次抓到我自己寫的兩條是假的：

- 六角數量 `>= 3` —— 數的是整頁的 `<i></i>`，但進度條與價格膠囊各有一個，六角砍到剩一個照樣湊得到 3。
- 標題比對用全文 `includes` —— 而 CSS 註解裡就寫著那句標題，兩頁標題互換也驗得過。

之後每一張卡都固定做這一步。

### ⚠ Day 50 之後要 FOX 處理的

1. **實機驗收**。這兩天改的十一個畫面我都**看不到** —— popup 無法被腳本開啟或截圖；`annotate` / `edit-report` 需要真實的截圖或錄製資料才進得去；注入 UI 要在真實網站上。
   - `checkout` / `day-pass-checkout` / `mic-permission` **可以直接用網址開**：`chrome-extension://<ID>/checkout.html`。
   - 其餘要實際走一次流程。
2. **`#stopBtn` 要不要改磚紅**（PM-423 的唯一未做項）。
3. **`bee.webm` 2.8MB 待壓**（本機無 ffmpeg）；`hornet-real.png` 在擴充功能已無引用但仍打包（71KB）。
4. **兩處 `innerHTML` 待清**：`mic-permission.ts` 的 `permDesc`、`edit-report.ts` 的 `renderTokenEstimate`。兩處都安全（來源是自家字典），但都是「不用 innerHTML」規則剩下的例外，各約 15 行。
5. **上架前跑 `npm run build`（不加 `--dev`）** —— 目前 `dist/` 是含 `<all_urls>` 的 DEV manifest。


## Day 49（2026-09-02）— PM-413~421，9 張卡：大黃蜂視覺系統 · 群組 A（Popup）

> 全套 **841 / 0**（14 套）。`_verify403` **93 → 122**（新增 29 條規格護欄）。
> **MCP 工具總數 65 支不變**（bridge 51 + 雲端 14）——這天只換視覺，沒有動任何 API、事件綁定或狀態機。
...
`popup.html` 的 `:root` 一次寫齊：§2.1 十四色、§2.4 深底次要文字的**兩個**色階、§3 字體三角色、§7 圓角、§7.1 硬投影、§4 蜂巢紋、§5 六角 `clip-path`。之後任何一個 class 都不再手寫色碼。

`body` 寬度 **360 → 320**（§9）。PM-409 當初加寬到 360 是為了讓中文標籤不斷行；320 站得住是靠三件事：標題與按鈕全 `nowrap` + `flex-shrink: 0`、三入口說明文字手動斷兩行、次要說明降級為括號小字。**`_verify403` 那條寫死 `width: 360px` 的斷言跟著翻面，但底下的 nowrap 與 flex-shrink 斷言一條都沒放寬**——那三招才是 320 能成立的前提。

🔴 **刻意不外連 Google Fonts。** §3 給了 `<link>`，但 popup 是擴充頁面：連外字型等於每開一次 popup 就向第三方送一次請求，隱私政策要交代（v1.1.5 已被 CWS 以「隱私政策資訊不足」退件過一次），離線還會先閃一次無樣式。改用「設計字型優先 + 系統字備援」的字型堆疊，**零網路請求**。代價是沒裝 Archivo／JetBrains Mono 的機器上字形不完全對版；要完全對版就把 woff2 打包進 `dist/` 再補 `@font-face`，三個變數不用動。**待 FOX 決定。**

### 九個畫面（PM-414~420）

| 卡 | 畫面 | 重點 |
|---|---|---|
| 414 | 01 登入 | `<bee-video>` 92px + `land`／`float` 動畫，靜態蜂圖疊底當備援；Google 鈕白底黑框 + 硬投影 |
| 415 | 18 主畫面 | 十三個區塊全換；emoji 從 markup **與字典**兩邊清 |
| 416 | 03/04 錄製中・完成 | 共用 `body.dark` 整頁反黑；46px 等寬計時 + 12 柱波形；完成頁米白摘要卡 |
| 417 | 19 進階設定展開 | 五列 1px 淡線；語言下拉做成米白卡；MCP 說明改咖啡底 |
| 418 | 20 偵察模式 | 第二層整頁反黑；圖釘嚴重度改 §7.7 左側 3px 色條 |
| 419 | 09~13 票券・付費・日票・額度・取消 | §7.5「倒數用黑底、額度用黃底」；§7.3「實線=已填、虛線=待填」 |
| 420 | 21/22 兩個 overlay | 麥克風黃卡 + 48px 六角麥克風；JSON 警語咖啡卡 + 脈衝警示三角 |

### 三個「照字面做會出事」的地方

- 🔴 **emoji 只清 markup 沒有用。** `applyTranslations()` 會把每個 `[data-i18n]` 元素的 `textContent` 換成字典值——字典裡的 emoji 會**蓋回**畫面上。清了 **55 個 popup 專用 key**（五語）。這 56 個 key `content.ts` / `inject.ts` / `annotate.ts` / `edit-report.ts` 一個都沒用到，群組 B／C 完全不受影響。另外，幾何圖示一律做成 `[data-i18n]` 元素的**兄弟節點**，翻譯覆蓋 `textContent` 時才不會把圖示一起洗掉。
- 🔴 **拿掉顏色圓點會讓資訊消失。** `scout-zone-fmt` 原本是 `{n} 區（🟢{ok} 🟡{warn} 🔴{err}）`，三個圓點一拿掉就是三個無法分辨的數字。依 §7.7「不靠顏色傳達內容」補上文字標籤：`{n} 區（正常 {ok}／注意 {warn}／異常 {err}）`。圖釘與巡檢結果同理——**判定訊號仍然是 content script 給的那顆 emoji（PM-405 的協定沒有動，popup 不重算嚴重度）**，只是改用 §7.7 的左側 3px 色條呈現。
- 🔴 **`body.dark` 是錄製中與偵察模式共用的。** 沒有護欄的話，錄製狀態一更新就會呼叫 `show('idle')`，把還在第二層的使用者的黑底關掉。所以 `showScout` 另外掛 `scout-open`，`show()` 看到它就不碰 `dark`。

### 順手修掉的既有問題（不在卡片範圍內，但改了就會踩到）

- **登入鈕**：`googleLoginBtn.textContent = ...` 會把按鈕裡的 Google logo `<img>` 一起洗掉。舊版是純文字鈕看不出來，新版 logo 是視覺重點——登入失敗一次 logo 就再也回不來。改成只換 `#googleLoginLabel`。
- **複製 MCP 的回饋卡**：原本 `style.display = 'block'`。新版回饋卡是 `display:flex` 加一個 `::before` 六角，而 **block 容器裡的 `::before` 預設是 inline、寬高會被忽略**，六角直接不見。改成固定用 `flex`，顏色由 class 決定。
- **更新通知**用 `innerHTML` 塞模板字串，改成 `createElement` + `textContent`（README 的既有規則就是 Trusted Types 相容寫法）。現在 `popup.ts` 全檔零 `innerHTML`，並加斷言釘住。
- **九句硬寫在 `.ts` 的中文**（上傳狀態三句、複製/匯出回饋三句、更新通知三句）在英文／日韓越模式下都只會顯示中文。全部收進字典。

### 擴充圖示（PM-421）

128 / 48 / 32 / 16 四枚重畫，比例照 `BugEzy Logo 方案.dc.html` 的尺規逐枚給值：外層黃色圓角方 → 內層黑色六角 → 核心 162° 斜紋六角。**16px 依 §5 去掉斜紋只留純六角**（那個尺寸下 3px 斜紋只會糊成一片）。用 8 倍超取樣再 LANCZOS 縮回目標尺寸。`manifest.json` 補上 32px。

### 29 條規格護欄（PM-421）

`_verify403` 新增一整段 `⑤ PM-413~421`，把 DESIGN_SPEC 裡**最容易被改回去、而且改回去不會有人立刻發現**的規則釘成斷言：

十四色 token 齊全 · 深底次要文字只有兩階 · 蜂巢紋 data URI 內無未編碼 `;` · 登入頁有影片備援 · `bee-video.js` 不走 esbuild entryPoints · 不外連 Google Fonts · **popup markup 零 emoji** · **popup 可觸及的字典值零 emoji** · 圖示是兄弟節點 · `body.dark` 有 `scout-open` 護欄 · 12 柱波形 · §7.3 虛線=待填 · §7.5 倒數黑底／額度黃底 · 警示三角沒有搭配 `box-shadow`（§5：會被裁掉）· `popup.ts` 零 `innerHTML` · 字標是 `BugEzy` 不是 `BUGEZY`。

### ⚠ 三個做不到 / 刻意不照做的，附理由

1. **錄製中的三格計數（Console / Network / DOM）沒有做。** 錄製期間 `GET_STATE` 的 `summary` 是 `null`，數字要等 `RECORDING_DONE` 把 content script 記憶體裡的資料合併完才產生；`chrome.storage.local` 的四個 buffer 只收跨頁 flush 的部分，單頁錄製時幾乎是空的，拿它顯示會給出**看起來像真的、其實嚴重少算**的數字。要做需要在 background 加一支 `GET_LIVE_COUNTS` + content script 定期回報——那是行為改動，README 明講「這次只換視覺」。**沒有留三個永遠是空的格子**（產品上看起來像壞掉）。
2. **日票沒有做成獨立的反黑畫面。** 設計稿畫面 11 只有倒數與升級鈕、**沒有三個錄製入口**；照做等於「持有日票時就換一個不能錄製的畫面」，那是導覽結構改動，而日票用戶正是最需要錄製入口的人。改成**倒數卡本身反黑**，§7.5「倒數 vs 額度用底色分辨緊急程度」有做到，第一層功能一個都沒少。
3. **登入頁背景用 `--y` 不是 `--cream`。** 卡片寫 `--cream`，但 §2.1 寫明 `--y` 是「主面板底」、`--cream` 是「資料卡、輸入框、已填欄位」，設計稿畫面 01 也是黃底。規範與設計稿一致、只有卡片不同。

### ⚠ Day 49 之後要 FOX 處理的

1. **重新載入擴充功能，然後把九個畫面看一遍。** popup 不是 content script，沒辦法用 jsdom 抽出來跑；那 122 項驗的是原始碼層面的不變式。**實際長什麼樣只有你看得到**，特別是 320px 下第二層標題會不會被裁。
2. **`bee.webm` 是 2.8MB 原樣複製的**（這台機器沒有 ffmpeg）。擴充包會從約 1.1MB 變成約 4MB。建議壓一版——92px 顯示用不到現在的位元率，換檔即可，程式碼不用動。
3. **要不要把 woff2 打包進 `dist/`**（見 PM-413）。
4. **上架前記得跑 `npm run build`（不加 `--dev`）**——目前 `dist/` 是 DEV manifest（含 `<all_urls>`），不可上架。


## Day 48（2026-08-21）— PM-403~411，9 張卡

> 全套 **954 / 0**（14 套）。**MCP 工具總數 65 支不變**（bridge 51 + 雲端 14）——這天全是 UI 重構與文件。
> Workers deploy `e61740a3`（guide）。🎉 **真 Chrome 端到端這天終於跑到了**：`_verify_phase1` **200 / 0**，是 Day 45 之後第一次（先前三批都因 port 19850 被 MCP server 占用而 SKIP）。

### popup 兩層架構重構（PM-403~405）

- **PM-403** 第一層只留日常功能，圖釘整段搬到第二層。**實作刻意保守**——沒有把 `idleView` 拆成兩個滑動面板（那裡掛了錄製／截圖／票券／提示詞／設定太多既有邏輯），而是新增同層的 `#scoutView`，用既有的 `.hidden` 切換。切頁動畫用 `animation` 而非 `transition`（元素從 `display:none` 出現，transition 沒有起始狀態可過渡）。
- **PM-404** 第二層：圖釘 + AI 監測 + 記憶矩陣。
  - 🔴 **卡片的記憶矩陣做不到，因為 bridge↔Extension 的 WebSocket 是單向的**（bridge 送命令、Extension 回結果，Extension 無法主動發問）。照字面做的話那個區塊會**永遠**顯示「Bridge 未連線」——連 bridge 連著時也一樣，等於在畫面上說謊。補了一條 **Extension → bridge 的唯讀查詢通道**：**硬編碼 switch 白名單**（只有 `memory_stats`）、未連線立刻回覆不重試、5 秒逾時。
  - 🔴 **[清除全部] 沒有做**：清空整層記憶是破壞性操作，走唯讀通道等於繞過 MCP 那邊的方案閘門與 `confirm: true` 防呆（PM-359 特地做的）。改為指引使用者用 AI 呼叫 `memory_clear`。**待決策。**
  - **AI 監測不是重寫偵測**：`start_auto_detect` 本來就只是編排既有工具，popup 依同樣順序重跑一遍並自行彙總，沒有複製任何判定邏輯。結果固定附上 30 秒視窗的說明。
- **PM-405** 巡檢／分析結果改為結構化區塊 + 依嚴重度上色。**顏色取自 content script 已算好的 emoji，popup 不重新判定嚴重度**（PM-350 的 score/summary 不同步已經吃過這個虧）。PM-393 的純文字版已移除，不留第二條顯示路徑。

### bugezy.dev/guide（PM-406）

🔴 **`/guide` 早就存在，這張卡是補內容不是建立**——照卡片草稿整頁重寫會是嚴重的內容倒退（現有頁面已有六種錄製模式、五種 AI 工具的 MCP 設定、雙語、sitemap 三語）。只加缺的：MCP30 領取章節、**`?token=` 卡關警告框**、四題 FAQ、Gemini 說明。

警告框存在的理由：沒 seed 過 localStorage 的使用者拿到的網址不帶 token，**功能一切正常、只有領券會失敗，而且完全看不出來**（PM-400 實測查到的阻塞點）。

### UI 修補三連（PM-408~410）——都是前一張卡造成的

- **PM-408** 翻譯。根因是 `t()` 找不到鍵時**回傳鍵本身**，所以 PM-403/404 漏了字典項時畫面直接顯示 `scout-mode`，**且沒有任何錯誤**。修的範圍比卡片列的六個鍵大得多：**第二層使用者實際讀到的文字大半是 popup.ts 動態產生的**，只補六個鍵的話英文模式仍是滿滿中文。共補 6 個 UI 鍵 + 43 條動態文字 × 5 語。順手把「拿另一個檔案產生的**中文句子**當判斷依據」改成穩定機器碼 `bridge_offline`。**並加守衛：每個 `data-i18n` 與 `t()` 用到的鍵都必須在字典裡、第二層不得殘留硬編碼中文。**
- **PM-409** 排版。加寬 320→360px 只是其中一環——**真正的原因是 flex item 預設 `min-width: auto`，空間不足時會先把文字折行而不是溢出**。標題 nowrap + 按鈕 `flex-shrink: 0` 一起做才修得掉。
- **PM-410** 🔴 **section 標題消失，是 PM-409 造成的**：那張卡為了做 ellipsis 給標題加了 `min-width: 0` + `overflow: hidden`——**正是「flex item 被壓到 0 寬、整個看不見」的典型組合**。取捨當時就想錯了：標題只有 3~5 個字，不該是會讓步的那一方。改成兩側都不讓步、真的擠不下時讓**容器**裁切。失敗模式從「元素憑空消失」變成「整列右邊被裁一點」，後者一眼看得出來。順手把三個 section 的結構統一（圖釘那段原本沒有分隔線）。

### 隱藏未實作功能（PM-411）

國際付款「即將開放」提示改由**單一開關** `SHOW_INTL_COMING_SOON = false` 關閉，四個顯示點全部套上。**不用 CSS 蓋掉**（會變成「程式說顯示、樣式說不顯示」兩個真相）、**不直接移除**（卡片明講之後要打開，且 PM-172 的 IP 國家判斷本身是對的，不該陪葬）。⚠ 副作用：非台灣使用者現在看不到任何付費相關內容，也不會知道為什麼——要不要補一句不帶承諾的說明，待決策。

### 驗收（PM-407）

新增 `_verify403.mjs`（**92 項**，涵蓋 PM-403~411）。全套 14 套：

| 項目 | 數字 |
|---|---|
| 端到端測試 | **954 項，0 failed** |
| MCP 工具總數 | **65**（bridge 51 + 雲端 14，**不變**） |
| 真 Chrome 端到端 | **200 / 0**（Day 45 後第一次跑到） |

### ⚠ Day 48 之後要 FOX 處理的

1. **重新載入擴充功能 ＋ 重連 MCP，兩個都要** —— 唯讀查詢通道的**兩端都改了**（bridge 的 `extension-link.ts` + extension 的 `background.ts`）。只重載擴充功能的話，記憶矩陣區塊會顯示「Bridge 未連線」。
2. **popup 的實機視覺仍需你看一眼** —— popup 不是 content script，沒辦法用 jsdom 抽出來跑，那 92 項驗的是原始碼層面的不變式（元素存在、事件有掛、CSS 規則寫對）。**PM-410 的標題有沒有真的出現，只有你看得到。**
3. **記憶矩陣的 [清除全部]** 要不要為此放寬唯讀通道。
4. **非台灣使用者的付費說明** 要不要補一句。

## Day 47（2026-08-20）— PM-398~401，4 張卡

> MCP 對接成功專屬的 30 天體驗券。**MCP 工具總數 65 支不變**（bridge 51 + 雲端 14）——這天沒有新增工具，做的是既有記錄機制補上使用者維度。
> Workers 已 deploy 兩次：`2c4c2713`（PM-398/399）→ `7acacf35`（PM-401）。

### MCP30 票券系統

- **PM-398** 建立 MCP30 + 兌換時驗證 MCP 使用紀錄。
  - 🔴 **卡片的核心前提不成立**：`mcp_usage` **原本沒有 `user_id` 欄位**（PM-380 的授權稽核已標紅過），所以卡片設計的 `.eq('user_id', userId)` **對每一個人都會查不到**。`logMcpUsage` 改為從 MCP URL 的 `?token=` 解出使用者並寫入。
  - 🔴 **寫入必須有退路**：`user_id` 要等 ALTER 跑完才存在，而 PostgREST 對未知欄位是整筆拒絕、`logMcpUsage` 的錯誤又是被吞掉的——先部署再跑 SQL 的話，**全站 MCP 用量記錄會靜靜停止寫入且沒人會發現**。做成「先帶 user_id 試一次，失敗退回原欄位組合」（同 PM-82 的處理方式）。
  - 🔴 **MCP 驗證擺在「重複兌換檢查」之後、「搶名額」之前**——反過來的話，資格不符的人會先把限量代碼的名額搶走再被拒絕。MCP30 雖不限量，但這段是所有代碼共用的。
  - 🔴 **查不出來時 fail closed**（503「無法驗證」），與「你還沒接 MCP」（403 + 教學連結 + `need_mcp` 旗標）**分成兩種訊息**——混在一起使用者會一直重試自己沒做錯的事。
  - `server/mcp30-ticket.sql`：卡片的 INSERT ＋ **卡片沒寫但缺了就不會動的 ALTER** ＋ 索引 ＋ 驗證查詢。
- **PM-399** 部署 + 端到端驗證。線上確認認證閘門、BUG10 行為一致、**未登入時不洩漏代碼是否存在**。
- **PM-400** 🔴 **實測查出真正的阻塞點：MCP 網址沒有帶 `?token=`。** `logMcpUsage` 解使用者的唯一來源就是它，所以**寫進去的每一筆都是 `user_id = NULL`，MCP30 永遠拿不到**。過程中順帶確認 `logMcpUsage` 本身正常（呼叫次數 109→110），代表 PM-398 的退路沒有把記錄寫壞。
- **PM-401** 補上第二條 token 來源：`logMcpUsage`／`txtWithTokens` 接受呼叫端**已驗證過**的 userId，四支會驗身分的工具傳入。
  - 🔴 **刻意不用「回填 `env.__mcp_session_token`」那個做法** —— `/mcp` 路由的 PM-190 註解正是在提醒避開那個競態樣式（同 isolate 併發互相覆寫）。傳明確的值沒有共享可變狀態，並加測試鎖住「原始碼不得出現對它的賦值」。
  - **只有比對過 token 屬於該使用者才帶 userId**；`list_reports` 在「查無此 email」時刻意不帶——那時根本還沒驗證身分。
  - 線上驗證：帶**故意寫錯**的 token 呼叫 → 「驗證失敗」且**連用量紀錄都不會產生**，自然不可能被錯誤歸戶。
...
|---|---|
| MCP 工具總數 | **65**（bridge 51 + 雲端 14，**不變**） |
| 新增測試 | `server/_verify398.mjs` **33 項**（server 端第一個驗收腳本：原始碼不變式 + 線上端點行為） |
| 票券 | BUG10（10 天）＋ MCP30（30 天）＝ 40 天體驗 |

### 🔴 Day 47 之後要 FOX 處理的

1. **把 `?token=` 加進 MCP 網址** —— `https://bugezy.dev/mcp?token=<session token>`（從擴充功能進階設定複製）。這是**最省事且覆蓋所有工具**的解法；PM-401 補的「用參數帶 token」那條路救不到 `get_usage_stats` 這種沒有 session_token 參數的工具。
   > 收工當下實測：`list_reports` 仍回「請在 MCP URL 加上 ?token=」，代表這個連結器**還沒有**帶 token。
2. **跑 `server/mcp30-ticket.sql`**（INSERT + ALTER 兩段都要）。若還沒跑，`with_user_id` 會一直是 0，而且**外在表現與「沒帶 token」一模一樣**——兩者都是「次數有加、歸戶是 0」。
3. 兌換 MCP30 實測（需要真實帳號，我沒有也不該有 session token）。

### 其他（FOX 手動完成，非本 session 驗證）

- **BUG10 票券建立**（Supabase SQL）
- **MCP30 雙向驗證**（無紀錄→被拒／有紀錄→成功）— FOX 回報；本 session 無法驗證，見上方待辦 1、2
- **Claude Taiwan 社團貼文定稿**（心得分享版 + 斥候行銷圖）

> PM-379（Supabase RLS 手動查核）已記錄在 **Day 45**，此處不重複計入。
