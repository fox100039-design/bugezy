...
## Day 48（2026-08-21）— popup 兩層架構 + guide MCP30（PM-403~407）

> 全套 **766 / 0**（14 套，新增 `_verify403` 47 項）。**MCP 工具總數 65 支不變。** Workers deploy `e61740a3`。

- **PM-403** popup 改為兩層：第一層只留日常功能，圖釘整段搬到第二層。**實作刻意保守**——沒有把 `idleView` 拆成兩個滑動面板（那裡掛了錄製／截圖／票券／提示詞／設定太多既有邏輯），而是新增同層的 `#scoutView`，用既有的 `.hidden` 切換。切頁動畫用 `animation` 而非 `transition`（元素從 `display:none` 出現，transition 沒有起始狀態可過渡）。
...
- **PM-406** 🔴 **`/guide` 早就存在，這張卡是補內容不是建立**——照卡片草稿整頁重寫會是嚴重的內容倒退。只加：MCP30 領取章節、**`?token=` 卡關警告框**（把 PM-400 查到的阻塞點接到使用者文件）、四題 FAQ、Gemini 說明。警告框存在的理由：沒 seed 過 localStorage 的使用者拿到的網址不帶 token，**功能一切正常、只有領券會失敗，而且完全看不出來**。
- **PM-407** 驗收收工。
