...
## Day 47（2026-08-20）— MCP30 票券（PM-398~399）

MCP 對接成功專屬的 30 天體驗券：兌換時檢查這個帳號**真的呼叫過 MCP**才發。已 deploy（`2c4c2713`）。

- 🔴 **`mcp_usage` 原本沒有 `user_id` 欄位**（PM-380 的稽核已標紅），所以卡片設計的 `.eq('user_id', userId)` **對每一個人都會查不到**。`logMcpUsage` 改為從 MCP URL 的 `?token=` 解出使用者並寫入。
...
- 🔴 **查不出來時 fail closed**（回 503「無法驗證」），與「你還沒接 MCP」（403 + 教學連結 + `need_mcp` 旗標）**分成兩種訊息**——混在一起使用者會一直重試自己沒做錯的事。
- `server/mcp30-ticket.sql`：卡片的 INSERT ＋ **卡片沒寫但缺了就不會動的 ALTER** ＋ 索引 ＋ 三段驗證查詢。
- ⚠ **舊資料無法回填**：`user_id` 是現在才加的，先前的 MCP 呼叫都沒有歸屬。包含 FOX 在內都要在部署後**再呼叫一次 MCP** 才會產生第一筆可驗證的紀錄。
- **PM-400／401** 🔴 **實測發現 MCP 網址沒帶 `?token=`，所以 `user_id` 永遠是 NULL、MCP30 永遠拿不到。** `logMcpUsage` 只看得到 URL token，看不到工具參數的 `session_token`——兩條 token 來源在「用量歸戶」上不等價，而使用者完全看不出差別。已補：`logMcpUsage`／`txtWithTokens` 接受呼叫端**已驗證過**的 userId，四支工具傳入。
  - **刻意不用「回填 `env.__mcp_session_token`」那個做法** —— `/mcp` 路由的 PM-190 註解正是在提醒避開那個競態樣式（同 isolate 併發互相覆寫）。傳明確的值沒有共享可變狀態，並加測試鎖住「原始碼不得出現對它的賦值」。
  - **只有比對過 token 屬於該使用者才帶 userId**；`list_reports` 在「查無此 email」時刻意不帶。
  - ⚠ 最省事且覆蓋所有工具的解法仍是**把 `?token=` 加進 MCP 網址**——`get_usage_stats` 根本沒有 session_token 參數，靠參數那條救不到它。
- 新增 `server/_verify398.mjs` **33 項**（server 端首個驗收腳本：原始碼不變式 + 線上端點行為）。全套 **708 / 0**。
