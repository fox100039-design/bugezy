...
第 5 代 Day 10（PM-71~72b）。Chrome Web Store 打包 + 綠界 ECPay 金流串接（測試環境跑通：單次付款 + 定期定額月訂閱）。CheckMacValue 依綠界官方 AI Skill 校正並對官方測試向量 + 線上獨立重算雙重驗證。
...
- 技術債：正式上線換 ECPAY 正式 key（HASH_KEY/IV 建議改 `wrangler secret`）；定期定額降級策略目前「任一期失敗即降 free」，宜改寬限期/連續失敗 N 次（綠界連續失敗 6 次才終止合約）；測試環境只扣第一期；升降級後用戶需重開 popup 才反映；PM-71 更新通知 + Day 9 三項打磨待實機驗收
