...
- 綠界 ECPay 金流（PM-72/72b，測試環境跑通）：`GET /checkout?user_id=` 回自動提交綠界表單（含定期定額 Period 參數，月扣 NT$80）→ 付款 → `POST /api/ecpay/callback`（首期）/`POST /api/ecpay/period-callback`（第 2 期起）驗 CheckMacValue → 更新 `users.plan`（成功 paid/失敗 free）→ `POST /checkout/result` 結果頁。popup 升級鈕開 `/checkout`。**CheckMacValue 踩雷**：依綠界官方 AI Skill `ECPay/ECPay-API-Skill`（`guides/13` + `test-vectors`）——TS 的 `encodeURIComponent` 不編碼 `~`/`'`，需額外 `~→%7e`、`'→%27`、`%20→+` 再小寫 + 還原 .NET 7 字元；Workers 無同步 crypto 改用 `crypto.subtle`（async SHA256）。**務必對官方 8 個測試向量驗證**。測試帳號 MerchantID 3002607（公開），正式上線換正式 key（HASH_KEY/IV 建議 `wrangler secret`）
