...
| ③ 能存能看 | Cloudflare Workers + Supabase + R2 + React 報告頁 | 🔨 進行中（API 完成，擴充上傳+報告頁待做） |
| ④ AI 能讀 | MCP Server（Pull 模式 8 Tool）= MVP 封測 | 待做 |
| ⑤ 能收錢 | Stripe 付費 + Chrome Web Store 上架 | 待做 |
| ⑥ 更好用 | 即時字幕、隱私遮罩、Groq Whisper 降級 | 待做 |
...
- 後端 API：Cloudflare Workers（✅ PM-10 骨架完成：POST/GET /api/reports）
- 資料庫：Supabase PostgreSQL + Auth（✅ reports schema 完成，待 FOX 建表）
- 檔案儲存：Cloudflare R2（✅ rrweb 存 reports/<id>/rrweb.json，待 FOX 建 bucket）
- Web 報告頁：React + Vite（待第 3 代 PM-12）
- MCP Server：TypeScript + MCP SDK 8 Tool Pull 模式（待第 4 代）
