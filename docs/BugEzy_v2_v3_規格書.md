...
> 📌 **所有操作類／偵測類工具都帶可選的 `tab_id?: number`**：省略 → 當前分頁（§13.1 偵察模式）；指定 → 只作用於該分頁（§13.2 出任務模式）。
...
> 🔴 **待決策：圖釘類與監控類要不要也吃 `tab_id`？** PM-300 只指定「操作類／偵測類」，故上面兩張表**維持原樣未改**。但這兩類本質上都綁定某個分頁（圖釘是畫在某頁的 DOM 覆蓋層、監控是收某頁的事件流），不補會留下歧義：出任務模式下 `start_monitor()` 到底監控誰？建議一併補上，語意與 §13.3 相同。**超出本卡片範圍，先標記不動手。**
