...
📌 工具總數連動：既有 13 ＋ v2 新增 17 ＋ 記憶 7 ＝ **37 個**（`get_live_errors` 不重複計）。
...
> 🔴 **Phase A 動工前必須先回答的三個問題（本節未定義）**
>
> - **記憶存在哪裡？** L1／L2／L4 記的都是**使用者原始碼的內容**（業務規則、架構細節、修法片段）。存雲端 Supabase → `/privacy` 得同步改（見 `ARCHITECTURE.md §4-15`：隱私政策要對得上程式碼）；存本機（bridge 旁）→ 換機、多人協作、L8 團隊共享全部要重新設計。**這個選擇會決定 Phase A 的整個資料層，不能邊做邊決定。**
> - **L1 跨不跨使用者共享？**「越用越聰明」若指全平台共享，威力最大——但一則經驗裡含著 A 公司的檔名、函式名與修法，被 B 公司的 AI 取用就是**原始碼細節外洩**。若不共享，新用戶的 L1 是空的，第一個月毫無感覺。**兩條路的產品體驗差很多。**
> - **L6／L7 的「主動撤回修改」由誰執行？** §14.6 寫「立刻自己改掉」、§14.7 寫「主動撤回修改」——這都是**寫入使用者程式碼**的動作。目前 v1／v2 的工具全是唯讀或操作瀏覽器，**沒有任何一支會改檔案**。要不要跨過這條線、以及要不要有人工確認關卡，屬產品層決策。
>
> 這三題都不影響本節「寫下規格」，但**會直接決定 Phase A 第一張卡怎麼開**，故先標記。
