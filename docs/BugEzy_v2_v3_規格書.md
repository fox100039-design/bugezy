...
| 2026-08-17 | **v1.4** | **Phase 5 完工**（PM-350~354）：嚴重度自動分類 §6、自訂規則、`start_auto_detect`／`get_detect_report`、`correlate_errors` 共 7 支；bridge 工具總數 **37** |
| 2026-08-17 | **v1.5** | **§14 記憶矩陣完工**（PM-355~361）：13 支記憶工具 ＋ `memory_stats`、`.bugezy/` 檔案系統；**新增 §14.13 實作註記**列出十處與原文的落差；bridge 工具總數 **51** |
| 2026-08-17 | **v1.6** | **Phase 6 PM-P／PM-Q 完工**（PM-362~364）：`tier-gate.ts` 涵蓋全部 51 支、Workers 端新增 `get_usage_quota`（雲端 MCP 13 → **14**）；並記錄兩處卡片與線上實際不符之處 |
| 2026-08-17 | **v1.7** | **兩輪安全審察與修復**（PM-365~381）：P0 WebSocket Origin 驗證、P1 終端機指令白名單與語音外洩、P2/P3 路徑正規化／限長／regex 守衛／綠界表單；per-user 授權稽核標紅兩項待決策。全站 MCP 工具 **65**（bridge 51 + 雲端 14）|
