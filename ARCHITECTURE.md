...
PM-384 的釘選模式在 **capture 階段攔所有 click**，PM-392 的探測會 **`el.click()` 模擬點擊**——兩個功能單獨看都對，合在一起就壞：使用者在 popup 按 [分析]，跳出來的卻是描述輸入框，而且那個模擬點擊被 `preventDefault` 吃掉，等於根本沒測到頁面的反應。

三道**互相獨立**的防線：

| 防線 | 範圍 | 做法 |
|---|---|---|
| ① `isProbing` | 只有 `el.click()` / `dispatchEvent` 那一瞬間 | 探測前後掛旗標，釘選 handler 看到就原樣放行 |
| ② 釘選後自動分析 | UX | 釘完立刻跑一次分析，使用者直接看到 🟢/🔴，不用再點 [分析] |
| ③ `pinModeSuspendDepth` | 整個分析／巡檢期間 | 用**計數器**而非布林——巡檢會對每個圖釘各跑一次分析，巢狀時不能被內層提前解除；且一律 `try/finally`，探測拋例外不會讓釘選模式永久卡在放行狀態 |

- **旗標放模組層而不是 `window.__bugezy_probing`**：content script 的 window 雖然是 isolated 的，掛上去沒有好處，只多一個會被誤改的表面。
- ⚠ 防線②有個必然的副作用：**剛被攔下來沒觸發的按鈕，會在自動分析時被探測點一次**。這是刻意的（釘一個按鈕的意思就是「幫我盯著它」），具破壞性的按鈕仍然會被 `looksDestructive()` 跳過。
...
| 2026-09-07 | **SEO 修正 + v1.2.0 送審**（PM-446~448）。GSC 報「已封鎖 robots.txt 無法索引」，查證後**不是缺路由** —— `/robots.txt` 與 `/sitemap.xml` PM-136／PM-264 就做好了（線上 200、Content-Type 正確、57 個 URL 帶完整 hreflang），照卡片重做會掉 hreflang／語言版本／部落格，還會拿掉保護使用者資料的 `Disallow: /report/`。**真正原因**：`/reports` 被全站 11 頁 footer 連結（PM-434 起放進共用 footer），同時被 robots.txt 擋住 —— 而它自己已送 `noindex, nofollow`，被擋住時 Google 永遠讀不到那個 noindex，既排不出索引又一直報警。**修法（方案 A）**：拿掉 `Disallow: /reports` 讓 Google 讀得到 noindex；`/report/:id` 補 `X-Robots-Tag: noindex, nofollow`（原本完全沒有 robots 標記）；`Disallow: /report/` 維持。**從 master 部署**（`4bd3dd24`），逐字套用而非 cherry-pick —— 整包 cherry-pick 會把分支上 PM-440 的 footer `v1.2.0` 一起上線，跟「`/api/version` 留在 1.1.5」自相矛盾。🔴 **回歸腳本對換行符敏感**：`git checkout` 後檔案變 CRLF，五條嵌了 `\n` 的斷言假紅，10 支腳本已加正規化。護欄：`_verify403` 第 ⑲ 區 5 條 + `_verify-live.mjs` 的線上 SEO 檢查（「公開頁 → 被擋路徑」已翻綠）。master **1007 / 0**、`release/v1.2.0` **594 / 0 / 53 skipped**。PM-305~448 共 **142 張**。 |
